...
object SerDeUtils extends JsonSupport {
  ...
  implicit def serialize[P <: AnyRef](value: P): Array[Byte] = {
    ...
    val valueSerialized: Array[Byte] =
      SerializationUtils.serialize(str.asInstanceOf[JSerializable])
    valueSerialized
  }

  implicit def deserialize[P <: AnyRef](
    value: Array[Byte]
  )(implicit m: Manifest[P]): Option[P] = {
    Option(value) match {
      case Some(v) => {
