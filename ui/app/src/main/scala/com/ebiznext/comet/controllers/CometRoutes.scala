      ...
      pathPrefix("clusters" / Segment) { clusterName =>
        pathEnd {
          post {
              ...
              fileUpload("inventory") {
                case (_, fileStream) =>
                  ...
                  onSuccess(writeResult) { result =>
                    result.status match {
                      case Success(_) =>
                        ClusterService.create(userId, Cluster(clusterName, targetPath))
            ...
            get {
              complete(ClusterService.retrieve(userId, clusterName))
            } ~
            delete {
          ...
          pathPrefix("nodes" / Segment / "tags" / Segment) { (nodeName, tagName) =>
            post {
              entity(as[TagValue]) { tagValue =>
                complete(NodeService.assignTag(userId, clusterName, nodeName, tagValue))
              }
            } ~
              delete {
                complete(NodeService.unassignTag(userId, clusterName, nodeName, tagName))
              } ~
              put {
                entity(as[Tag]) { tag =>
                  complete(TagService.update(userId, clusterName, tagName, tag))
