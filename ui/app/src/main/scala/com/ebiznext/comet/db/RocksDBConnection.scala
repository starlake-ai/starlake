...
class RocksDBConnection(config: RocksDBConfig) extends DBConnection {
  import SerDeUtils._
  lazy val db: RocksDB = RocksDB.open(config.toOptions, config.path)
  ...
  override def read[V <: AnyRef](
      key: String
  )(implicit m: Manifest[V]): Option[V] = this.synchronized {
    deserialize[V](db.get(serialize[String](key)))
  }

  override def write[V <: AnyRef](key: String, value: V)(
