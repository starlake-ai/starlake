...
trait RocksDBConnectionSpecUtils {
  ...
  class RocksDBConnectionMock extends RocksDBConnectionLike {
    ...
    override def read[V <: AnyRef](
      key: String
    )(implicit m: Manifest[V]): Option[V] =
      RocksDBConnectionMock.dbMocked.get(key).asInstanceOf[Option[V]]
    override def write[V <: AnyRef](key: String,
                                    value: V)(implicit m: Manifest[V]): Unit =
      RocksDBConnectionMock.dbMocked.put(key, value)
    override def delete(key: String): Unit =
      RocksDBConnectionMock.dbMocked.remove(key)
  }
...
trait RocksDBConnectionBaseSpec
  ...
  implicit lazy val rocksdbConnection: RocksDBConnectionLike =
    new RocksDBConnectionLike()
...
trait RocksDBConnectionMockBaseSpec
  ...
  implicit lazy val rocksdbConnection: RocksDBConnectionMock =
    new RocksDBConnectionMock()
  lazy val rocksDb: mutable.HashMap[String, AnyRef] =
    RocksDBConnectionMock.dbMocked
