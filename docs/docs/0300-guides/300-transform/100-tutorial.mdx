...
```bash
starlake lineage --recursive --name kpi.order_summary
```
...
starlake lineage --name kpi.order_summary --recursive --svg --output lineage.svg
```
...
```bash
starlake transform --recusrive --name kpi.order_summary
```
