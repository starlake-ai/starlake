...
## SELECT statements
SQL transform are SELECT statements materialized using a user defined strategy.
...
The only requirement is that the result of the SELECT statement must match the schema of the target table if ti exists, otherwise starlake will infer the schema.


:::warning

The `*` character is not allowed in the SELECT statement.
