...
Transform tests are located in the `meatdata/tests/transform` directory.

Each test is a directory located in the `domain/table` subdirectory and contains the following files:
- multiple CSV of JSON files that will contain the initial data that will be loaded into the tables before the unit test is run.
