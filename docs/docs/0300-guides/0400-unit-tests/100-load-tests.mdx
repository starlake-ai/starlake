...
Load tests are located in the `meatdata/tests/load` directory.

Each test is a directory located in the `domain/table` subdirectory and contains the following files:
- a CSV of JSON file that will contain the initial data that will be loaded into the table before the unit test is run.
