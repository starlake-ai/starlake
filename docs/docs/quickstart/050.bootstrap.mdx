...
- The example below set the active connectionRef using the `connection`variable defined in the environment configuration file (see [Environment configuration](env))
...
That's it, we are ready to go. Let's start by [loading some data](load) into our warehouse and then [transforming it](transform) to make it ready for analysis.
