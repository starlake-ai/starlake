...
To install a specific version (0.5.3 in the example below), set the SLK_VERSION environment variable before running starlake.
...
```sh
$ SLK_VERSION=0.5.3 ./starlake.sh # version 0.5.3 will be installed
...
```powershell
c:\users\me\starlake> SET SLK_VERSION=0.5.3
c:\users\me\starlake> starlake
...
```shell
$ docker build --build-arg SLK_VERSION=0.5.3 -t starlake .
$
```
