...
To install a specific version (0.5.2 in the example below), set the COMET_VERSION environment variable before running starlake.
...
```powershell
c:\users\me\starlake> SET COMET_VERSION=0.5.2
c:\users\me\starlake> starlake
...
```shell
$ docker build --build-arg COMET_VERSION=0.5.2 -t starlake .
$
```
