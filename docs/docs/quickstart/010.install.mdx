...
```sh
$ SL_VERSION=0.5.3 ./starlake.sh # version 0.5.3 will be installed
...
```powershell
c:\users\me\starlake> SET SL_VERSION=0.5.3
c:\users\me\starlake> starlake
...
```shell
$ docker build --build-arg SL_VERSION=0.5.3 -t starlake .
$
```
