...
This SQL file alone is not sufficient to be a Starlake Transform. It instructs Starlake to put the result of the query in a schema (dataset in BigQuery)
...
This instructs to run all transformations in `kpi` dataset  in append mode.
...
To illustrate how transform may be defined dynamic table names, we will create two tables, one containing customers living in France
