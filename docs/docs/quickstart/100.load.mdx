...
This command will infer the YAML configuration file used by the loader to watch the folder for new files to load and to validate each record in those files.
...
The domain configuration file __config.comet.yml_ is stored in the _domain_ folder.
...
```
You may change this path in the _directory_ attribute to any other path.
...
In other words, we have the following environments
...
### Import step
In this first step, Starlake will look at the _directory_ attribute value in the YAML file and look at the file that matches the expected pattern by the table definition.
In our example, the directory is `{{root_path}}/incoming` and the expected file pattern has been changed to `customers-.*.psv`
`import` move files from domain directory defined in any domain files such as `metadata/load/sales/_config.comet.yml` to the `datasets/pending` directory.
...
In our case this variable is set as follows to the _LOCAL_ value.
...
env:
  ...
  SL_ENV: LOCAL
  myConnectionRef: "???"
  bigQueryMaterializationDataset: "???" # need to be defined for spark validator on BigQuery only
  validator: "???" # native or spark. Need to be defined in the specific env.{{SL_ENV}}.comet.yml file
```
...
env:
  ...
  validator: "spark" # spark since we are running in local mode
```

The ```connectionRef``` refers to the connection defined in the `application.comet.yml` file as follows:
...
application:
  connections:
    ...
    bigquery:
      type: "bigquery"
      options:
        gcsBucket: "starlake-app" # Temporary GCS Bucket where intermediary files will be stored. Used by the spark validator only.
  ...
  validator: "{{ validator }}" # native or spark depending on the env.LOCAL.comet.yml & env.BQ.comet.yml files
  spark:
    datasource:
      bigquery:
        materializationDataset: {{ bigQueryMaterializationDataset }} # need to be defined for spark validator on BigQuery only

```


### Loading the data into BigQuery
To load the data into BigQuery, simply put back the customer-*.psv file in the incoming folder and set
```SL_ENV=BQ``` in the _env.comet.yml_ file and run the load command again.
...
env:
  ...
  validator: "native" # native of spark # Load using the BigQuery API
```

To load the file using Spark simply set the validator to spark as follows:
...
env:
  ...
  validator: "spark" # Use Spark as the ingestion engine
```
