...
You may also use _${ }_ to define variables but this is not recommended.
...
To load the data into BigQuery, simply put back the customer-*.psv file in the coming folder and set
...
application:
  ...
  validator: "spark" # native of spark
  spark:
    datasource:
      bigquery:
...
and set the values specifi to BigQuery in the metadata/env.BQ.comet.yml file as follows:
...
- It allows to load data from any source supported by Spark
- It allows to load data into any destination supported by Spark
