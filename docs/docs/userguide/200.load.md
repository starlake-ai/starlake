...
Load rules are stored in the folder referenced by the SL_METADATA
environment variable (/tmp/metadata by default).

:::note

You need to export the SL_METADATA variable before executing any comet load step.
...
Types are defined in the file $SL_METADATA/types/types.comet.yml.
...
located in the file $SL_METADATA/types/default.comet.yml and they may be redefined
in the file $SL_METADATA/types/types.comet.yml

File ``$SL_METADATA/types/default.comet.yml``
...
$SL_METADATA/types/types.comet.yml
...
$SL_METADATA/domains. They defined :
...
File ``$SL_METADATA/domains/sales.yml``
...
aka $SL_METADATA/domains/sales.yml.
...
File ``$SL_METADATA/load/sales/_config.yml``
...
```yaml title="Schema definition file customers.comet.yml"
table:
  name: "customers"
...
Below, the complete tables definition files.
...
### How it works
The Load process will scan the all domain pending folders in the cluster.
...
invoked through request submitted to a job manager by at the Load Step.
