...
Load rules are stored in the folder SL_ROOT/metadata folder.

:::note

You need to export the SL_ROOT variable before executing any comet load step.
...
File ``$SL_ROOT/metadata/types/default.comet.yml``
...
### How it works
1. On startup, all the domain definitions files are loaded from the folder /tmp/metadata/load
