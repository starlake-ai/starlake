...
This works the same way as for the load time upserts.

Say we did not use and upsert strategy at load time and that after appending the data to the `sales.customers` table,
we want to create a table `silver.customers_last_signup` containing the most recent data for each customer. 
We can do this using the `writeStrategy` attribute in the transformation configuration file.
