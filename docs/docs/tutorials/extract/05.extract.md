...
Database extraction consist in extracting the table schemas and extracting the data.

## Schema Extraction

Schemas extraction is automated through the CLI using the `extract` command.

This command is on a YAML file that describe :
- The connection to use as specified in the application.conf file located in the `metadata` directory.
- The list of tables to extract
- For each table you may restrict the columns you want to extract by specfiying the columns to include in the extraction process
- Whether you want to extract tables, views  or aliases

This result in YAML files that describe your data source. 
These YAML files will be used to create your target database schema  
