...
|----------------------|----------------|-----------------|---------------------|
| Support large volumes of data | __Yes. Also note that starlake can take advantage of all the processing power to parallelize the extraction process__ | No, since data are pulled one record at a the over the network| __Yes. data is extracted in near realtime__ |
