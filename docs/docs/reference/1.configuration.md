...
You may also pass any Spark arguments as usual but also pass a custom `application.comet.yml` file .
...
and [reference-*.conf](https://github.com/starlake-ai/starlake/tree/master/src/main/resources) files. In your `application.comet.yml`file you only
...
An `application.comet.yml` file stored in the metadata subdirectory will be automatically loaded by Starlake. 
...
````
    ...
    `-- metadata (Root of metadata files)
        ...
        |   |-- hr/_config.comet.yml (One definition file / domain)
        |   `-- hr/sales.yml
...
Below is the default HOCON file for filesystem options:
```hocon
file-system = "file://"
file-system = ${?SL_FS}
