...
```python
    create_cluster = DataprocClusterCreateOperator(
        ...
        properties = {
            "spark-env:SLK_FS": "gs://${my_bucket}",
            "spark-env:SLK_HIVE": "false",
            "spark-env:SLK_GROUPED": "false",
...
t1 = dataproc_operator.DataProcSparkOperator(
  ...
  dataproc_spark_properties={'spark.driver.extraJavaOptions':'-DSLK_FS=gs://${my_bucket} -DSLK_HIVE=false -DSLK_GROUPED=false'},
  dag=dag)
```


Starlake allows you to override some configurations properties using predefined env variables prefixed by `SLK_`. 
...
|:--------------|:------------|:-------|:-----------
|file-system|SLK_FS|file://|File system where datasets will be located
|metadata-file-system|SLK_METADATA_FS|${file-system}|File system where Starlake metadata will be located
|root|SLK_ROOT|/tmp|Root directory of the datasets and metadata files in the defined filesystem above
|tmp-dir|SLK_TMPDIR|When compacting data and estimating number of partitions, Starlake stores intermediates files in this folder|${root}"/comet_tmp"
|datasets|SLK_DATASETS|${root}"/datasets"|Folder where datasets are located in the datasets `file-system`
|metadata|SLK_METADATA|${root}"/metadata" otherwise|Folder where metadata are located in the metadata `metadata-file-system`
|file-system|SLK_FS|file://|File system where datasets will be located
|area.pending|SLK_AREA_PENDING|pending|Source files are copied here from the incoming folder before processing
|area.unresolved|SLK_AREA_UNRESOLVED|unresolved|Files found in the incoming folder but do not match any schema
|area.archive|SLK_AREA_ARCHIVE|archive|Source files as found in the incoming folder are saved here after processing
|area.ingesting|SLK_AREA_INGESTING|ingesting|Temporary folder used during ingestion by Starlake
|area.accepted|SLK_AREA_ACCEPTED|accepted|root folder of all valid records
|area.rejected|SLK_AREA_REJECTED|rejected|invalid records in processed datasets are stored here
|area.business|SLK_AREA_BUSINESS|business|root folder for all datasets produced by autojobs
|archive|SLK_ARCHIVE|true|Should we archive the incoming files once they are ingested
|default-write-format|SLK_DEFAULT_WRITE_FORMAT|parquet|How accepted records are stored (parquet / orc / json / csv / avro)
|default-rejected-write-format|SLK_DEFAULT_REJECTED_WRITE_FORMAT|parquet|How rejected records are stored (parquet / orc / json / csv / avro)
|default-audit-write-format|SLK_DEFAULT_AUDIT_WRITE_FORMAT|parquet|How audit is stored (parquet / orc / json / csv / avro)
|hive|SLK_HIVE|true|Should we create external Hive tables for ingested files ?
|analyze|SLK_ANALYZE|true|Should we computed basic statistics ? (requires SLK_HIVE to be set to true)
|launcher|SLK_LAUNCHER|simple|Which orchestrator to use ? Valid values are airflow or simple (direct call)
...
metadata-file-system = ${?SLK_METADATA_FS}
...
datasets = ${?SLK_DATASETS}

metadata = ${root}"/metadata"
metadata = ${?SLK_METADATA}

area {
  pending = "pending"
  pending = ${?SLK_AREA_PENDING}
  unresolved = "unresolved"
  unresolved = ${?SLK_AREA_UNRESOLVED}
  archive = "archive"
  archive = ${?SLK_AREA_ARCHIVE}
  ingesting = "ingesting"
  ingesting = ${?SLK_AREA_INGESTING}
  accepted = "accepted"
  accepted = ${?SLK_AREA_ACCEPTED}
  rejected = "rejected"
  rejected = ${?SLK_AREA_REJECTED}
  business = "business"
  business = ${?SLK_AREA_BUSINESS}
}
...
default-write-format = ${?SLK_DEFAULT_WRITE_FORMAT}

default-rejected-write-format = parquet
default-rejected-write-format = ${?SLK_DEFAULT_REJECTED_WRITE_FORMAT}

default-audit-write-format = parquet
default-audit-write-format = ${?SLK_DEFAULT_AUDIT_WRITE_FORMAT}

launcher = airflow
launcher = simple
launcher = ${?SLK_LAUNCHER}
```
...
|HOCON Variable|Env variable|Default Value|Description
|:--------------|:------------|:-------|:-----------
|lock.path|SLK_LOCK_PATH|${root}"/locks"|Root folder where lock file is created
|lock.timeout|SLK_LOCK_TIMEOUT|-1|How long to wait for the file lock to be available (in seconds)
...
When ingesting the files with the same schema one after the other, it is possible to use a custom ordering policy by settings the `SLK_LOAD_STRATEGY` environment variable. Currently, the following ordering policies are defined:
...
If you want to use another custom strategy, you'll have to implement the trait below, make it available in the classpath and set the `SLK_LOAD_STRATEGY` environment variable
...
To ingest all the files at once, set the `SLK_GROUPED` variable to true.

|HOCON Variable|Env variable|Default Value|Description
|:--------------|:------------|:-------|:-----------
|grouped|SLK_GROUPED|false|Should files with the same schema be ingested all at once ?
|load-strategy-class|SLK_LOAD_STRATEGY|ai.starlake.job.load.IngestionTimeStrategy|When `grouped` is false, which ingestion order strategy to use 
...
load-strategy-class = ${?SLK_LOAD_STRATEGY}
...
set the `SLK_SINK_TO_FILE` environment variable to `true`.

|HOCON Variable|Env variable|Default Value|Description
|:--------------|:------------|:-------|:-----------
|sink-to-file|SLK_SINK_TO_FILE|false|Should ingested files be stored on the filesystem on only in the sink defined in the YAML file ?

```hocon
sink-to-file = false
sink-to-file = ${?SLK_SINK_TO_FILE}
```

When `sink to file` or a filesystem sink (SinkType.FS) is requested, and you want to output the result in a single file in the csv file format, set the `SLK_CSV_OUTPUT` 
...
:---|:---|:---
row-validator-class|SLK_ROW_VALIDATOR_CLASS|ai.starlake.job.validator.FlatRowValidator
tree-validator-class|SLK_TREE_VALIDATOR_CLASS|ai.starlake.job.validator.TreeRowValidator
...
|HOCON Variable|Env variable|Default Value|Description
|:--------------|:------------|:-------|:-----------
||AIRFLOW_ENDPOINT|Airflow endpoint. Used when SLK_LAUNCHER is set to airflow|http://127.0.0.1:8080/api/experimental
...
CometImport = BashOperator(
    ...
    bash_command= SLK_SPARK_CMD + ' import',
    env={
        'SLK_DATASETS':"/project/data",
        'SLK_METADATA':"/project/metadata",
        'SLK_AREA_ACCEPTED':"working",
        'SLK_AREA_PENDING':"staging",
        'SLK_ARCHIVE':"true",
        'SLK_LAUNCHER':"airflow",
        'SLK_HIVE':"true",
        'SLK_ANALYZE':"true"
    },
    dag=dag)
...
SLK_DOMAIN = os.environ.get('SLK_DOMAIN', '')
CometWatch = BashOperator(
    task_id='comet_watcher',
    bash_command= SLK_SPARK_CMD + ' watch '+ SLK_DOMAIN,
...
SLK_SPARK_CMD = "spark2-submit \
                        ...
                        --principal importhdfs@MY.BIGDATA \
                        --conf spark.jars.packages=\"\" \
                        --conf spark.yarn.appMasterEnv.SLK_METADATA=/project/metadata \
                        --conf spark.yarn.appMasterEnv.SLK_ACCEPTED=working \
                        --conf spark.yarn.appMasterEnv.SLK_DATASETS=/project/data \
                        --master yarn \
                        --deploy-mode cluster /home/airflow/program/comet-assembly-0.1.jar"

templated_command = SLK_SPARK_CMD + """ {{ dag_run.conf['command'] }}"""
