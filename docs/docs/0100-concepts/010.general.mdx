...
This allows to go much further than simply checking the input against standard SQL types.
...
For example, we may want to check that the number of records loaded in a table is greater than a certain threshold.
...
The refs file allows us to make our query more readable by allowing to map the name `my-table`.

- __metadata/env.{{SL_ENV}}.sl.yml__: we may use any of the variables defined in the env files in our extract, load and transforms jobs.
...
For example, if we have a table defined in the following path: `metadata/transform/my_domain/my_table.sl.yml`, then the domain name will be `my_domain`.
...
Any load or transform job that will end up writing to a table `my_table` in the domain `my_domain`
is defined in the file `metadata/load/my_domain/my_table.sl.yml` for load jobs and in the file `metadata/transform/my_domain/my_table.sl.yml` for transform jobs.

:::note

We may still overload the table name in the file `my_table.sl.yml` by specifying a different table name in the `name` property.
...
For example, if we have a table `my_table` in the domain `my_domain`, we may reference it in our SQL query by using the syntax `my_table` instead of `my_domain.my_table` or `my_database.my_domain.my_table`.
