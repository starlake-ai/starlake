...
Let's say we want to load the data coming from a file or extracted from the previous example into a datawarehouse
...
table:
  ...
  metadata:
    ...
    writeStrategy:
      type: "SCD2"      # Use the Slow CHanging Dimension Type 2 Strategy when writing to the destination table.
      timestamp: signup
      key: [id]
...
Let's say we want to build aggregates from existing tables or from the previously loaded files
...
Starlake will take care of generating the corresponding DAG (Directed Acyclic Graph) and will run it on your requested orchestrator (Airflow, Dagster ...)
