...
tags: [Spark, BigQuery, Dataproc, Google Cloud, ETL]
---
...
```scala {4,5,8} title="Delete partitions that need to be updated"
val incomingDF = ... // Incoming data loaded with the correct schema
incomingDF
  ...
  .map(_.getString(0))
  .foreach { partition =>
    spark.sql(s"DELETE $table WHERE date  like $partition")
  }
```
...
## Comments
[Let's continue the conversion on Discord](https://discord.com/channels/833336395430625310/908709208025858079)
