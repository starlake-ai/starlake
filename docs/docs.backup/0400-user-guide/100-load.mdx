...
$ docker run                                                     \
    -e SL_ROOT=/app/userguide                                    \
    -e SL_DEFAULT_WRITE_FORMAT=parquet                           \
...
This will load the data files and since we chose the localFilesystem connection, they will be stored them as delta/parquet files into the following folders:
...
When storing locally, Starlake uses the delta file format, to store the data in the parquet file format, set the environment variable `SL_DEFAULT_WRITE_FORMAT` to `parquet`
...
You can check the result by running the following python script to read the parquet files from the project directory:
...
## Incremental load
Sometimes, we want to alter existing data by adding new records or updating existing ones.

This is useful when we have a large amount of data and we want to avoid reloading the entire dataset every time we want to update our data warehouse.
...
This is done using the `writeStrategy` attribute in the table configuration file. In this example, we will upsert records in the `sales.customers`.

The `writeStrategy.timestamp` attribute is used to recognize the most recent record concerning a customer identified by the columns listed in `writeStrategy.key` attribute.
If the `writeStrategy.timestamp` attribute is not specified, new records (the records being loaded) are considered to be the most recent ones.
In our case, the `signup` date will be used as the timestamp for the records identified by their column `id`.


This is the content of the `metadata/load/customers.sl.yml` file once the `writeStrategy` attribute is added (highlighted lines):

```yaml title="metadata/load/sales/customers.sl.yml" {15-17}
---
table:
...
Using Spark instead of the BigQuery Load API has among others the following advantages:
...
Even when using Spark, you do not need to instantiate a cluster. The Spark mode is useful for loading diverse file formats and advanced data validation at load time.
