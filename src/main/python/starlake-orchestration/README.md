...
The `ai.starlake.job.airflow.StarlakeAirflowJob` class is also responsible for recording the `outlets` related to the execution of each starlake command, usefull for scheduling DAGs using **data-aware scheduling**.
...
from ai.starlake.job.airflow.bash import StarlakeAirflowBashJob
...
This instance is available in the `cluster` property of the `StarlakeAirflowDataprocJob` class and can be configured using the `ai.starlake.job.airflow.gcp.StarlakeDataprocClusterConfig` class.
...
from ai.starlake.job.airflow.gcp import StarlakeAirflowDataprocJob
...
from ai.starlake.job.airflow.gcp import StarlakeAirflowCloudRunJob
