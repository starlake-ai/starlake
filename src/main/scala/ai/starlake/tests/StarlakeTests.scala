...
case class StarlakeTestData(
  ...
  def load(conn: java.sql.Connection): Unit = {
    if (createTableExpression.nonEmpty) {
      ...
      Console.println("*********************************************************")
      Console.println(createTableExpression)
      Console.println("*********************************************************")
      stmt.close()
    }
  }
...
// Call transpile
            ...
            if (isDataFile) {
              ...
              val expectedCreateTable: String =
                loadDataAsCreateTableExpression(
                  ...
                  f,
                  None // Some(taskOrTableFolderName)
                )
  ...
  private def loadDataAsCreateTableExpression(
    ...
    domainName: TableOrTaskName,
    tableName: TableOrTaskName,
    dataFile: File,
    taskOrTableFolderName: Option[TableOrTaskName]
  ) = {

    val domain = schemaHandler.domains().find(_.finalName == domainName)
    val table = domain.flatMap { d =>
      d.tables.find(_.finalName == taskOrTableFolderName.getOrElse(tableName))
    }
