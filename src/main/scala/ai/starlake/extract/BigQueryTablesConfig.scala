  ...
  val parser: OParser[Unit, BigQueryTablesConfig] = {
    ...
    OParser.sequence(
      ...
      opt[Seq[String]]("tables")
        .action { (x, c) =>
<<<<<<< HEAD:src/main/scala/ai/starlake/schema/generator/BigQueryTablesConfig.scala
          val tables = x.map(_.split(".")).map(tab => tab(0) -> tab(1)).groupBy(_._1).map {
            case (k, v) => (k, v.map(_._2))
          }
          c.copy(tables = tables.view.mapValues(_.toList).toMap)
=======
          c.copy(tables = buildTablesMap(x))
>>>>>>> master:src/main/scala/ai/starlake/extract/BigQueryTablesConfig.scala
        }
        .optional()
