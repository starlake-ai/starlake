...
import ai.starlake.schema.generator.{ExtractBigQuerySchema, ExtractBigQuerySchemaCmd}
...
class ExtractSchema(schemaHandler: SchemaHandler) extends ExtractPathHelper with LazyLogging {
  ...
  def extractTable(
    ...
    extractTable(s"$domain.$table", connectionName)
  ...
  /** Generate YML file from JDBC Schema stored in a YML file
    *
    * @param userConfig
    *   - User configuration for extraction
    * @param settings
  ...
  def extract(userConfig: ExtractSchemaConfig)(implicit
    ...
    val connectionSettings: Connection = jdbcSchemas.connectionRef match {
      case Some(connectionRef) =>
        settings.appConfig.getConnection(connectionRef)
      case None =>
    ...
    if (connectionSettings.isBigQuery()) {
      val extractedDomains =
        jdbcSchemas.jdbcSchemas.flatMap { jdbcSchema =>
          val bigQueryConfig =
  ...
  private def fromConfig(
  ...
  )(implicit settings: Settings): JDBCSchemas = {
    ...
    if (config.all) {
      ...
      val schemaNames = ParUtils.withExecutor() { ec =>
        ...
        ExtractSchema.extractSchemaNames(Some(connectionRef))
      }
      val result =
        schemaNames match {
          case Failure(e) =>
          ...
          case Success(schemaNames) =>
            ...
            JDBCSchemas(jdbcSchemas = jdbcSchemas, connectionRef = Some(connectionRef))
        }
      result
    } else {
      ...
      if (config.tables.nonEmpty) {
        ...
        JDBCSchemas(jdbcSchemas = jdbcSchemas, connectionRef = config.connectionRef)
      } else {
  ...
  private def extractDomain(
    ...
    implicit val forkJoinTaskSupport: Option[ForkJoinTaskSupport] =
      ExtractUtils.createForkSupport(minForPar = 1) // FIXME
...
object ExtractSchema {
  ...
  def extractSchemaNames(
    ...
    connection.`type` match {
      case Some(connType) =>
        ConnectionType.fromString(connType) match {
          /*case ConnectionType.JDBC => // TODO
      ...
      case _ =>
        Try {
          throw new IllegalArgumentException(s"No connection type specified for $connectionName")
        }
    }
  }
}
