...
import ai.starlake.extract.ExtractSchemas
...
class SchemaHandler(storage: StorageHandler, cliEnv: Map[String, String] = Map.empty)(implicit
  ...
  def deserializedExtractConfigs(extractPath: Path): Map[String, ExtractSchemas] = {
    ...
    extractConfigsPaths.map { extractConfigPath =>
      ...
      val extractConfigFileContent =
        storage.read(extractConfigPath).richFormat(activeEnvVars(), Map.empty)
  ...
  def loadExtractConfigs(): Map[String, ExtractSchemas] = {
    if (storage.exists(DatasetArea.extract)) {
      ...
      Map.empty[String, ExtractSchemas]
    }
  }
