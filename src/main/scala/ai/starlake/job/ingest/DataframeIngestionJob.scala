...
class DataframeIngestionJob(
  ...
  override def loadDataSet(withSchema: Boolean): Try[DataFrame] = {
    Try {
      SparkUtils.runPySpark(
        new Path(mergedMetadata.loader.getOrElse(throw new Exception("Loader required"))),
        Map.empty
      )
      assert(session.catalog.tableExists("SL_THIS"))
      val dfIn = session.sqlContext.table("SL_THIS")
