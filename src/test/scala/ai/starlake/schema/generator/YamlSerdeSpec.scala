...
object YamlConfigGenerators {
  ...
  implicit val metadata: Arbitrary[Metadata] = Arbitrary {
    for {
      ...
      extensions    <- arbitrary[List[String]]
  ...
  implicit val privacyLevel: Arbitrary[PrivacyLevel] = Arbitrary {
    for {
      value <- arbitrary[String]
      sql   <- arbitrary[Boolean]
    } yield PrivacyLevel(value, sql)
  }
