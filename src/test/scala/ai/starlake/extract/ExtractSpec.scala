      ...
      jdbcSchemas shouldBe JDBCSchemas(
        ...
        jdbcSchemas = List(
          JDBCSchema(
            ...
            schema = "public",
            tables = List(
              JDBCTable("user", List("id", "email"), None, 1, Map.empty, None),
              JDBCTable("product", Nil, None, 1, Map.empty, None),
              JDBCTable("*", Nil, None, 1, Map.empty, None)
            ),
            tableTypes = List(
              "TABLE",
              "VIEW",
    ...
    new WithSettings() {
      ...
      ExtractSchema.extractSchema(
        JDBCSchema(
          ...
          None,
          None,
          List(JDBCTable("TEST_TABLE1", List("ID"), None, 1, Map.empty, None))
        ).fillWithDefaultValues(),
        settings.comet.connections("test-h2").options,
        File("/tmp"),
        None,
    ...
    new WithSettings() {
      ...
      ExtractSchema.extractSchema(
        JDBCSchema(
          ...
          None,
          None,
          List(JDBCTable("TEST_TABLE2", Nil, None, 1, Map.empty, None))
        ).fillWithDefaultValues(),
        settings.comet.connections("test-h2").options,
        File("/tmp"),
        None,
