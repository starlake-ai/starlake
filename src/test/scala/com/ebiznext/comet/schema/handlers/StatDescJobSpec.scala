...
class StatDescJobSpec extends FlatSpec {
  ...
  val result0 = Metrics.computeContinuiousMetric(dataInitialUsed, listContnuousAttributes, Metrics.continuousMetrics)

  val result1 = Metrics.computeContinuiousMetric(dataInitialUsed, listContnuousAttributes, partialContinuousMetric)
  ...
  val meanList: List[Double] = listContnuousAttributes.map(name => dataInitialUsed.select(avg(name)).first().getDouble(0))
  ...
  val minList: List[Double] = listContnuousAttributes.map(name => dataInitialUsed.select(min(name)).first().getString(0).toDouble)
  ...
  /**
   ...
   val maxList: List[Double] = listContnuousAttributes.map(name => dataInitialUsed.select(max(name)).first().getString(0).toDouble)
  ...
  val stddevList: List[Double] = listContnuousAttributes.map(name => dataInitialUsed.select(stddev(name)).first().getDouble(0))
  ...
  val skewnessList: List[Double] = listContnuousAttributes.map(name => dataInitialUsed.select(skewness(name)).first().getDouble(0))
  ...
  val kurtosisList: List[Double] = listContnuousAttributes.map(name => dataInitialUsed.select(kurtosis(name)).first().getDouble(0))
