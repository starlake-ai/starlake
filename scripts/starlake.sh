COMET_VERSION="${COMET_VERSION:-0.3.26}"
SPARK_VERSION="${SPARK_VERSION:-3.3.0}"
HADOOP_VERSION="${HADOOP_VERSION:-3}"
SPARK_BQ_VERSION="${SPARK_BQ_VERSION:-0.2.7-preview}"
SCALA_VERSION=2.12
...
echo "Installing binaries in $TARGET_FOLDER"
...
initEnv() {
  ...
  if test -f "$TARGET_FOLDER/bin/spark/bin/spark-submit"; then
      echo "spark folder found in $TARGET_FOLDER/bin/"
  else
  ...
  if test -f "$TARGET_FOLDER/$COMET_JAR_FULL_NAME"; then
      ...
      echo "$COMET_JAR_NAME found in $TARGET_FOLDER/bin/"
  else
  ...
  if test -f "$TARGET_FOLDER/$SPARK_BQ_JAR_FULL_NAME"; then
      echo $SPARK_BQ_JAR_FULL_NAME
      echo "$SPARK_BQ_JAR_FULL_NAME found in $TARGET_FOLDER/bin/"
  else
  ...
  if test -f $SPARK_SUBMIT; then
      echo "$SPARK_SUBMIT found in $TARGET_FOLDER/bin/"
...
run() {
  SPARK_DRIVER_OPTIONS="-Dlog4j.configuration=file://$TARGET_FOLDER/bin/spark/conf/log4j.properties.template"
