object Versions {
  val spark4 = "4.0.0"
  val deltaSpark4d0 = "4.0.0rc1"
  val scalatest = "3.2.19"
  val scalacheckForScalatest = "3.2.18.0"
  val typesafeConfig = "1.4.3"
  val scalaLogging = "3.9.5"
  val swaggerParser = "2.1.28"
  val betterFiles = "3.9.2"
  val jacksonForSpark4 = "2.17.2"
  val pureConfig = "0.17.9"
  val esSpark = "8.16.3"
  // starting 1.4.1, it uses 2.17.0 of jackson which is not compatible with provided version in spark
  val jsonSchemaValidator = "1.4.0"
  val scopt = "4.1.0"
  val bigquery = "2.49.0"
  // val gcsConnector = "3.0.4"
  val gcsConnector = "hadoop3-2.2.27"
  val hadoop = "3.4.0"
  val sparkBigqueryWithDependencies = "0.42.3"
  val bigqueryConnector = "hadoop3-1.2.0"
  val h2 = "2.3.232" // Test only
  val poi = "4.1.2"
  val scalate = "1.9.8"
  val kafkaClients = "7.7.3-ce"
  val confluentVersion = "7.7.3"
  val testContainers = "0.41.8"
  val gcpDataCatalog = "1.69.0"
  val gcpCloudLogging = "3.21.4"
  val jinja = "2.8.0" // forces dependency override on guava
  val snowflakeJDBC = "3.23.2"
  val snowflakeSpark: String = "3.1.1"
  val jSqlParser = "5.4-SNAPSHOT"
  val duckdb = "1.3.0.0"
  val jSqlTranspiler = "0.7-SNAPSHOT"
  val bigQueue = "0.7.0"
  val redshiftJDBC = "2.1.0.32"
  val scalaCompat = "2.12.0"
  val scalaParallelCollections = "1.0.4"
  val derbyVersion =
    "10.15.2.0" // last version compatible with Java 11, see https://db.apache.org/derby/derby_downloads.html
  val jSqlFormatter = "5.1-SNAPSHOT"
}
