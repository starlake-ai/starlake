object Versions {
  val sparkAvro = "4.0.0"
  val curator = "2.6.0"
  val spark2d4 = "2.4.8"
  val spark3d0 = "3.2.0"
  val scalatest = "3.2.10"
  val sparkXMLForScala2d11 = "0.13.0"
  val sparkXMLForScala2d12 = "0.14.0"
  val springBoot = "2.0.6.RELEASE"
  val typesafeConfig = "1.4.1"
  val scalaLogging = "3.9.4"
  val zookeeper = "3.4.6"
  val jets3t = "0.9.3"
  val hive = "3.1.0"
  val log4s = "1.3.3"
  val betterFiles = "3.9.1"
  val jackson211ForSpark2 = "2.6.7"
  val jackson212ForSpark3 = "2.10.0"
  val configsForScala3 = "0.6.1"
  val configsForScala2 = "0.4.4"
  val esSpark211 = "7.8.1"
  val esSpark212 = "7.15.0"
  val scopt = "4.0.1"
  val sttp = "1.7.2"
  val gcsConnector = "hadoop3-2.2.2"
  val bigqueryConnector = "hadoop3-1.0.1"
  val bigquery = "1.137.2"
  val hadoop = "3.3.1"
  val h2 = "1.4.200" // Test only
  val poi = "4.1.2"
  val scalate = "1.9.6"
  val akkaHttp = "10.1.14"
  //  val akkaStream = "2.6.12"
  val akkaStream = "2.5.32"
  val kafkaClients = "2.8.1"
  val testContainers = "0.39.10"
  val jsqlparser = "4.2"
  val silencerVersion = "1.7.1"
}
