...
class SchemaHandlerSpec extends FlatSpec with Matchers with Data {
  ...
  Area.init(storageHandler)
  
  val sh = new HdfsStorageHandler
  val domainsPath = new Path(Area.domains, domain.name +".json")
  sh.write(jswrite(domain), domainsPath)
  val typesPath = new Path(Area.types, "types.json")
  sh.write(jswrite(types), typesPath)

  Area.initDomains(storageHandler, schemaHandler.domains.map(_.name))
  ...
  "Ingest CSV" should "produce file in accepted" in {
    ...
    val targetPath = Area.path(Area.pending("DOMAIN"), "SCHEMA-VALID.dsv")
