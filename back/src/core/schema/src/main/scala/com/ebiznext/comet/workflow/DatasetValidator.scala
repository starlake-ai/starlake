...
class DatasetValidator(storageHandler: StorageHandler,
  ...
  def pending(
    ...
    val paths = storageHandler.list(Area.pending(domainName))
  ...
  def ingesting(domain: Domain, schema: SchemaModel.Schema, path: Path): Unit = {
    val targetPath = new Path(Area.ingesting(domain.name), path.getName)
  ...
  def run(): Unit = {
    ...
    domains.foreach { domain =>
      val (resolved, unresolved) = pending(domain.name)
        unresolved.foreach {
        case (_, path) =>
          val targetPath = new Path(Area.unresolved(domain.name), path.getName)
